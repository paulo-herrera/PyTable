...
from .ttypes import ALLOWED_TYPES 
...
class Table:
    ...
    # TODO: Implement tail and head for table.
    def print(self, maxRows: int = -1, writeTitle = True, out = sys.stdout, \
              sep = "\t", columnWidth = 10, missing = "-", verbose = False):
    ...
    def write(self, dst: str, sep = "\t", columnWidth = 10, missing = "-", verbose = False):
        ...
        self.print(writeTitle = False, out = sdst, sep = sep, \
                   columnWidth = columnWidth, missing = missing, verbose = verbose)
        sdst.close()
        
        
    def convert(self, cols: List[int], types: List[str]):
        """ Attempt to convert each column of this table to the specified type provided in the list fmt.
            ...
            NOTE: Specific formats to convert dates, floats, etc. can be specified with
                  setFormatStr.
        # """
        for t in types:
    ...
    @staticmethod
    def read(src: str, sep: str=",", header=1, removeEmptyColumn=False, \
             verbose=True, encoding: str = "utf-8", allowRepetition = True):
        ...
        strs = read_tab_file(src, sep, strip=True, verbose=False, encoding = encoding)
        nlines= len(strs)
        ncols = len(strs[0])
        if verbose:
